...
def bnid_generation_postprocess():
    ...
    `bnile-cdw-prod.dag_migration_test.bnid_dedup_fin` AS
    SELECT
    *
    FROM (
    SELECT
    ...
    FROM (
        ...
        duplicate_bnid
        FROM
        `bnile-cdw-prod.dag_migration_test.bnid_dedup_merge` ))
    WHERE
    primary_cnt=1
    ...
    drop table if exists `bnile-cdw-prod.dag_migration_test.final_bnid_table_dedup`;

    create table `bnile-cdw-prod.dag_migration_test.final_bnid_table_dedup`
    as
    select 
    ifnull(map.primary_bnid,f.bnid) as primary_bnid,
    f.*      
    from 
    `bnile-cdw-prod.dag_migration_test.final_bnid_table` f
    left join
    (
    SELECT
    distinct primary_bnid,duplicate_bnid
    FROM
    `bnile-cdw-prod.dag_migration_test.bnid_dedup_map`
    )map
    ...
    create or replace table `bnile-cdw-prod.dag_migration_test.up_email_address_dim_d`
    as
    select
    ...
    from
    `bnile-cdw-prod.dag_migration_test.up_email_address_dim` f
    left join
    (
    SELECT
    distinct primary_bnid,duplicate_bnid
    ...
    create or replace table  `bnile-cdw-prod.dag_migration_test.up_customer_attributes`  as
    SELECT
    ...
    v_to_v_flag
    FROM
    `bnile-cdw-prod.dag_migration_test.final_bnid_table_dedup`f
    left join
    ...
    left join
    `bnile-cdw-prod.dag_migration_test.up_email_address_dim` m
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.bnid_guid_distinct` AS
    SELECT
    ifnull(map.primary_bnid,f.bnid) AS bnid,
    --f.bnid AS original_bnid,
    customer_guid,
    ...
    SELECT
        ...
        COUNT(*) AS event_cnt
    FROM
        `bnile-cdw-prod.dag_migration_test.bnid_guid_mapping`
    GROUP BY
        bnid,
        customer_guid ) f
    LEFT JOIN (
    SELECT
        ...
        duplicate_bnid
    FROM
        `bnile-cdw-prod.dag_migration_test.bnid_dedup_fin` )map
    ON
    ...
    CREATE OR REPLACE TABLE `bnile-cdw-prod.dag_migration_test.bn_email_guid` as
    SELECT
    *
    FROM (
    SELECT
    ...
    JOIN (
        ...
        *
        FROM
        `bnile-cdw-prod.dag_migration_test.up_email_address_dim_d`) ead
    ON
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.bn_guid_all` AS
    ...
    guid
    FROM
    `bnile-cdw-prod.dag_migration_test.bn_email_guid`
    UNION DISTINCT
    ...
    FROM
        `bnile-cdw-prod.dag_migration_test.bnid_guid_distinct`
    WHERE
        rf_rank=1 )
    );	
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.click_fact_sku_views` AS
    (SELECT
    *
    FROM (
    SELECT
    ...
    CREATE OR REPLACE TABLE `bnile-cdw-prod.dag_migration_test.bn_daily_product_views` AS
    SELECT
    bgm.bnid,
    date_key,
    psd.product_set,
    ...
    FROM
    `bnile-cdw-prod.dag_migration_test.click_fact_sku_views` cf
    INNER JOIN (
    ...
    INNER JOIN
    `bnile-cdw-prod.dag_migration_test.bn_guid_all` bgm
    ON
    gd.guid=bgm.guid
    LEFT JOIN
