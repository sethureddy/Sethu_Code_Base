def bnid_generation_preprocess():
    ...
    drop table if exists `bnile-cdw-prod.dag_migration_test.master_event_stage`;
    CREATE TABLE IF NOT EXISTS
    `bnile-cdw-prod.dag_migration_test.master_event_stage` as
    (
    select
    event_type
    ,basket_id
  ...
  ,timestamp(event_date) as event_date
   ...
   over ( partition by phone_number order by case when birth_date is not null then 1 else 0 end desc, event_date desc, birth_date) as birth_date
    ...
    CREATE TABLE IF NOT EXISTS
    `bnile-cdw-prod.dag_migration_test.master_bn_table`(bnid	STRING
    ,phone_number	STRING		
    ...
    insert into `bnile-cdw-prod.dag_migration_test.master_bn_table`
    ...
    --NULL  as email_address,
    from
    (select distinct phone_number from `bnile-cdw-prod.dag_migration_test.master_event_stage` where email_address  is  null 
    and phone_number is not null  
    AND  PHONE_NUMBER NOT IN(select distinct COALESCE(phone_number,'AAAAAAAA') from `bnile-cdw-prod.dag_migration_test.master_event_stage`
    WHERE EMAIL_ADDRESS IS NOT NULL)
    )aa where aa.phone_number  not in(select distinct phone_number from `bnile-cdw-prod.dag_migration_test.master_bn_table` where email_address is null and phone_number is not null)
    union distinct
    select 
    concat('BN',GENERATE_UUID()) as bnid,
    null as phone_number,
    ...
    --NULL  as email_address,
    from
    (select distinct REGEXP_replace(REGEXP_replace(REGEXP_replace(REGEXP_replace(REGEXP_replace(trim(lower(email_address)),'gmial.com$','gmail.com'),'gimal.com$','gmail.com'),'yahooo.com$','yahoo.com'),'yyahooo.com$','yahoo.com'),'yyahoo.com$*','yahoo.com') as email_address from `bnile-cdw-prod.dag_migration_test.master_event_stage` where email_address  is not null )bb
    where lower(trim(bb.email_address)) not in(select distinct lower(trim(email_address)) from `bnile-cdw-prod.dag_migration_test.master_bn_table` where email_address is not null)
    )cc;

    ----- insert daily data and create bn_test_key strore in master_bn_test_key---------

    insert into `bnile-cdw-prod.dag_migration_test.master_bn_test_key`
    select bnid,
    ...
    from 
        ...
        ROW_NUMBER() OVER() + (
                                SELECT
                                    ...
                                    0)
                                FROM
                                    `bnile-cdw-prod.dag_migration_test.master_bn_test_key`) AS row_num



        FROM
        `bnile-cdw-prod.dag_migration_test.master_bn_table`
        WHERE
        bnid NOT IN (
        SELECT
            DISTINCT bnid
        FROM
            `bnile-cdw-prod.dag_migration_test.master_bn_test_key`)
    ...
    drop table if exists `bnile-cdw-prod.dag_migration_test.bnid_guid_mapping`;

    CREATE TABLE IF NOT EXISTS `bnile-cdw-prod.dag_migration_test.bnid_guid_mapping` as
    ...
    from
    (select distinct bnid,email_address,created_date,updated_date from `bnile-cdw-prod.dag_migration_test.master_bn_table`) b 
    join
    (
    select distinct event_type,email_address,customer_guid
    ,event_date
    from
    `bnile-cdw-prod.dag_migration_test.master_event_stage`
    ...
    insert into `bnile-cdw-prod.dag_migration_test.bnid_guid_mapping`
    ...
    ,s.event_date,b.created_date,b.updated_date
    from
    (select bnid,email_address,phone_number,created_date,updated_date from `bnile-cdw-prod.dag_migration_test.master_bn_table` where email_address is null and phone_number is not null ) b 
    join
    (
    ...
    drop table if exists `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping`;

    CREATE TABLE IF NOT EXISTS `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping` as
    (select 
    distinct 
    bnid,
    aa.event_type,
    ...
    FROM   `bnile-cdw-prod.dag_migration_test.master_event_stage` where email_address is not null)aa
    inner join
    (select bnid,email_address from `bnile-cdw-prod.dag_migration_test.master_bn_table`)bb 
    on lower(trim(aa.email_address))=lower(trim(bb.email_address)) where aa.email_address is not null);


    insert into `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping`
    ...
    FROM   `bnile-cdw-prod.dag_migration_test.master_event_stage` where email_address is null and phone_number is not null )aa
    inner join
    (select bnid,phone_number,email_address from `bnile-cdw-prod.dag_migration_test.master_bn_table`)bb 
    ...
    ----- Query for bnid_pii_mapping_cleaned ----- 
     ...
     drop table if exists `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping_cleaned`;

   CREATE TABLE IF NOT EXISTS `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping_cleaned` as
    ...
    cast(concat(ifnull(replace(bill_to_STREET_ADDRESS_1,'Â¿',''),'!'),',',ifnull(replace(bill_to_STREET_ADDRESS_2,'Â¿',''),'!'),',',ifnull(replace(bill_to_STREET_ADDRESS_3,'Â¿',''),'!')) as string) as bill_to_street,
    bill_to_CITY, 
    case when length(bill_to_STATE) = 1 then replace(replace(bill_to_STATE, "-",''),'.','')
    else replace(replace(bill_to_STATE, "-",''), 'Â¿','')
    end bill_to_STATE,
    bill_to_country_name,
    ...
    (replace(replace(bill_to_POSTAL_CODE,'""',''),')',''),'(',''),'@',''),'`',''),'=',''),'&',''),',',''),':',''),';',''),'?',''),'*',''),'#',''), '$',''), '!',''),'.','') ,'"',''),'+',''),'%',''),"'",''), '[',''), ']',''), '^',''),'<',''),'â€',''), '{',''), "\'", ''), '_','') bill_to_POSTAL_CODE,
...
-- five digits followed by three charecters converted to five digits
  ...
  --removing the special charecters   
     ...
     case when length(ship_to_STATE) = 1 then replace(replace(bill_to_STATE, "-",''),'.','')
      else replace(replace(replace(ship_to_STATE, "-",''), 'Â¿',''), '.','')
      end ship_to_STATE,
      ship_to_country_name,
      ship_to_country_code,
      replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace
     (replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace
     (replace(replace(ship_to_POSTAL_CODE,'""',''),')',''),'(',''),'@',''),'`',''),'=',''),'&',''),',',''),':',''),';',''),'?',''),'*',''),'#',''), '$',''), '!',''),'.','') ,'"',''),'+',''),'%',''),"'",''), '[',''), ']',''), '^',''),'<',''),'â€',''), '{',''),'_','')  as ship_to_POSTAL_CODE,
...
-- five digits followed by three charecters converted to five digits
      ...
      replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace
     (replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace
     (replace(replace(ship_to_POSTAL_CODE,'""',''),')',''),'(',''),'@',''),'`',''),'=',''),'&',''),',',''),':',''),';',''),'?',''),'*',''),'#',''), '$',''), '!',''),'.','') ,'"',''),'+',''),'%',''),"'",''), '[',''), ']',''), '^',''),'<',''),'â€',''), '{',''),'_','')
      end
      ship_to_postal_code_modified,
      event_date

     FROM `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping` )aa)bb 
    ...
    CREATE OR REPLACE TABLE  `bnile-cdw-prod.dag_migration_test.bnid_stage_table` as
    ...
    *,
    ...
    from `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping_cleaned`   order by phone_number_priority,first_name_priority,last_name_priority,birth_date_priority,gender_priority,Last_4_CC_priority,
    ...
    drop table if exists `bnile-cdw-prod.dag_migration_test.final_bnid_table`;
    CREATE TABLE IF NOT EXISTS `bnile-cdw-prod.dag_migration_test.final_bnid_table` as
    ...
    FROM `bnile-cdw-prod.dag_migration_test.bnid_stage_table`)aa)bb 
    ...
    CREATE TABLE IF NOT EXISTS
    `bnile-cdw-prod.dag_migration_test.up_email_address_dim`
    (up_email_address_key int64
    ,email_address	STRING
    ,bnid STRING
    );

    insert into `bnile-cdw-prod.dag_migration_test.up_email_address_dim`

    select 
    ROW_NUMBER() OVER(order by email_address) + (select coalesce(max(up_email_address_key),1000000) from  `bnile-cdw-prod.dag_migration_test.up_email_address_dim`) AS up_email_address_key,
    email_address,
    bnid
    from `bnile-cdw-prod.dag_migration_test.final_bnid_table` where email_address is not null and email_address not in(
    select distinct email_address from  `bnile-cdw-prod.dag_migration_test.up_email_address_dim`);
    ...
    `bnile-cdw-prod.dag_migration_test.dedup_phone_base1` AS
    SELECT
    bnid,
    phone,
    first_name,
    ...
    FROM (
        SELECT
            ...
            event_date
        FROM
            `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping_cleaned`   -- Check dataset path
        WHERE
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_phone_cnt1` AS
    SELECT
    *
    FROM (
    SELECT
    ...
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_phone_base1` )
    WHERE
    match_cnt>1
    AND match_cnt<=9
    ORDER BY
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_phone_map1` AS
    -- Create dup map
    SELECT
    ...
    SELECT
        *
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_phone_cnt1`
    WHERE
        row_num=1) primary
    RIGHT JOIN (
    SELECT
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_phone_base2` AS
    SELECT
    bnid,
    phone,
    first_name,
    ...
    SELECT
        ...
        event_date
    FROM
        `bnile-cdw-prod.dag_migration_test.bnid_pii_mapping_cleaned`
    WHERE
        phone_number IS NOT NULL
        AND first_name IS NOT NULL
    ...
    -- summary cnts 
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_phone_cnt2` AS
    SELECT
    *
    FROM (
    SELECT
    ...
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_phone_base2` )
    WHERE
    match_cnt>1
    AND match_cnt<=9
    ORDER BY
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_phone_map2` AS
    SELECT
    primary.bnid AS primary_bnid,
    primary.first_event_date AS p_first_event_date,
    dup.bnid duplicate_bnid,
    ...
    SELECT
        *
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_phone_cnt2`
    WHERE
        row_num=1) primary
    RIGHT JOIN (
    SELECT
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_cc_base1` AS
    SELECT
    bnid,
    last_4_digits,
    bill_to_POSTAL_CODE,
    ...
    CREATE OR REPLACE TABLE 
    `bnile-cdw-prod.dag_migration_test.dedup_cc_cnt1` AS
    SELECT
    *
    FROM (
    SELECT
    ...
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_cc_base1` )
    WHERE
    match_cnt>1
    AND match_cnt<=9
    ORDER BY
    ...
    --CC dedup map
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_cc_map1` AS
    SELECT
    primary.bnid AS primary_bnid,
    primary.first_event_date AS p_first_event_date,
    dup.bnid duplicate_bnid,
    ...
    SELECT
        *
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_cc_cnt1`
    WHERE
        row_num=1) primary
    RIGHT JOIN (
    SELECT
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_addr_base1` AS
    SELECT 
    bnid,
    bill_to_street,
    bill_to_POSTAL_CODE,
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_addr_cnt1` AS
    SELECT
    *
    FROM (
    SELECT
    ...
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_addr_base1` )
    WHERE
    match_cnt>1
    AND match_cnt<=9
    ;
    ...
    -- address dedup map
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_addr_map1` AS
    SELECT
    primary.bnid AS primary_bnid,
    primary.first_event_date AS p_first_event_date,
    dup.bnid duplicate_bnid,
    ...
    SELECT
        *
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_addr_cnt1`
    WHERE
        row_num=1) primary
    RIGHT JOIN (
    SELECT
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_guid_base1` AS
    SELECT 
    bnid,
    first_name,
    last_name,
    ...
    INNER JOIN
    `bnile-cdw-prod.dag_migration_test.bnid_guid_distinct` B
    ON
    a.bnid=b.bnid)
    GROUP BY
    bnid,
    ...
    --counts
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_guid_cnt1` AS
    SELECT
    *
    FROM (
    SELECT
    ...
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_guid_base1` )
    WHERE
    match_cnt>1
    AND match_cnt<=9
    ORDER BY
    ...
    CREATE OR REPLACE TABLE
    `bnile-cdw-prod.dag_migration_test.dedup_guid_map1` AS
    SELECT
    primary.bnid AS primary_bnid,
    primary.first_event_date AS p_first_event_date,
    dup.bnid duplicate_bnid,
    ...
    SELECT
        *
    FROM
        `bnile-cdw-prod.dag_migration_test.dedup_guid_cnt1`
    WHERE
        row_num=1) primary
    RIGHT JOIN (
    SELECT
    ...
    --- combine the maps
    create or replace table `bnile-cdw-prod.dag_migration_test.bnid_dedup_map` as

    select distinct primary_bnid, p_first_event_date, duplicate_bnid, d_first_event_date, rule
    from
    (
    select * from `bnile-cdw-prod.dag_migration_test.dedup_phone_map1`
    UNION ALL
    select * from `bnile-cdw-prod.dag_migration_test.dedup_phone_map2`
    UNION ALL
    select * from `bnile-cdw-prod.dag_migration_test.dedup_cc_map1`
    UNION ALL
    select * from `bnile-cdw-prod.dag_migration_test.dedup_addr_map1`
    UNION ALL
    select * from `bnile-cdw-prod.dag_migration_test.dedup_guid_map1`
    )
