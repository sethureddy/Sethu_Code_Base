def bn_customer_explicit_process():
    bql = '''
	create or replace table `bnile-cdw-prod.up_stg_customer.bnid_pii_mapping_cleaned_dedup`
	as
	select 
	...
	from 
	`bnile-cdw-prod.up_stg_customer.bnid_pii_mapping_cleaned` f
	left join
	(
	SELECT
	...
	CREATE OR REPLACE TABLE `bnile-cdw-prod.up_stg_customer.stg_final_customer_bnid` as
	(
	select 
	 bnid
	,first_name
    ...
    *,
	...
	from `bnile-cdw-prod.up_stg_customer.bnid_pii_mapping_cleaned_dedup`  order by phone_number_priority,first_name_priority,last_name_priority,birth_date_priority,gender_priority,Last_4_CC_priority,
	...
	CREATE OR REPLACE TABLE  `bnile-cdw-prod.o_customer.bn_customer_profile`
	as
	...
	FROM `bnile-cdw-prod.up_stg_customer.stg_final_customer_bnid` f
	left join
	...
	CREATE OR REPLACE TABLE  `bnile-cdw-prod.o_customer.bn_explicit_attributes` 
	as
