def explicit_process():
    ...
    CREATE OR REPLACE TABLE `bnile-cdw-prod.up_stg_customer.stg_pii_attributes` as
     (
	select distinct
    ...
    FROM 
    (select * from `bnile-cdw-prod.o_customer.up_customer_attributes`) h
    full outer join
    ...
    FROM
      `bnile-cdw-prod.o_customer.up_customer_attributes` bn
    full outer join
    ...
    from `bnile-cdw-prod.up_stg_customer.master_bn_test_key`
    )btk
  ...
  UNION ALL (
    ...
    WHERE
      ...
      UNION ALL (
   SELECT
      DISTINCT 'TheKnotWWData' AS event_type,
      4 AS rank,
      email_address,
      weddingdate,
      DATE(wedding_date_timestamp) AS wedding_date_timestamp,
    FROM (
      SELECT
        ...
        weddingdate ,
        DateOptined AS wedding_date_timestamp,
        row_number () OVER(PARTITION BY email ORDER BY DateOptined DESC) rn
      FROM
        `bnile-cdw-prod.theknotww_data.stg_theknotww_data`
      WHERE
        email IS NOT NULL
...
FROM ((
  ...
  UNION ALL (
     ...
     (select * from
        (
          (select email_address ,bill_to_postal_code,ship_to_postal_code,
                  ...
                  event_type, 
                  event_date  
           FROM `bnile-cdw-prod.up_stg_customer.bnid_pii_mapping_cleaned` )
    ...
    `bnile-cdw-prod.o_customer.up_customer_attributes`  where email_address is not null) b
    ...
    CREATE OR REPLACE TABLE `bnile-cdw-prod.up_stg_customer.stg_order_attributes`  as
    (
    ...
    (select up_email_address_key,bnid,email_address from 
    `bnile-cdw-prod.o_customer.up_customer_attributes`
    ...
    CREATE OR REPLACE TABLE `bnile-cdw-prod.up_stg_customer.stg_browse_attributes` as
     (
    ...
    CREATE OR REPLACE TABLE `bnile-cdw-prod.up_stg_customer.stg_flag_attributes` as
    ...
    `bnile-cdw-prod.up_stg_customer.stg_order_attributes`
...
join
`bnile-cdw-prod.o_customer.up_customer_attributes` b
...
group by email_address
    ...
    full outer join
     ...
     `bnile-cdw-prod.up_stg_customer.stg_browse_attributes`)
     using(email_address)),
    ...
    full outer join
    `bnile-cdw-prod.up_stg_customer.stg_browse_attributes` stg_b
    on stg_b.email_address = act.email_address
    full outer join
    `bnile-cdw-prod.up_stg_customer.stg_order_attributes` stg_o
    on stg_b.email_address =  stg_o.email_address
    full outer join
    `bnile-cdw-prod.up_stg_customer.stg_pii_attributes` stg_p
    ...
    CREATE OR REPLACE TABLE `bnile-cdw-prod.o_customer.up_explicit_attributes` as
    (
    select distinct
    up_email_address_key
    ,bnid
    ...
    ,marital_status_timestamp
    ,proposal_date
    FROM `bnile-cdw-prod.up_stg_customer.stg_pii_attributes`) p 
    ...
    FROM `bnile-cdw-prod.up_stg_customer.stg_order_attributes`) o
    ...
    FROM `bnile-cdw-prod.up_stg_customer.stg_flag_attributes`) f
    ...
    FROM `bnile-cdw-prod.up_stg_customer.stg_browse_attributes`) b
