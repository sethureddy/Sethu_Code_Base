def pj_rfm():
    ...
    drop table if exists `bnile-cdw-prod.dag_migration_test.bn_pj_tagging`;
    
    CREATE TABLE IF NOT EXISTS `bnile-cdw-prod.dag_migration_test.bn_pj_tagging` as
    ...
    WITH customer_data AS (
        ...
        ,pgd.product_intent
            ...
            join `bnile-cdw-prod.dag_migration_test.bn_guid_all` ead
    ...
    where 
        ...
        and  bnid not in (select bnid from `bnile-cdw-prod.dag_migration_test.stg_order_attributes` where order_cnt > 0)
    )  
    ),
    ...
    and	 bnid  in
    (
    select bnid from `bnile-cdw-prod.dag_migration_test.stg_order_attributes`
    where
    ...
    --- Rule 1 - Logic for last prior purchases
        and 
        bnid  in (select bnid from `bnile-cdw-prod.dag_migration_test.stg_order_attributes` where order_cnt > 0 )
        
        and bnid not in (select bnid from `bnile-cdw-prod.dag_migration_test.stg_order_attributes` where
    parse_DATE('%Y-%m-%d', CAST(order_engagement_last_dt AS string)) >= DATE_SUB(CURRENT_DATE (), INTERVAL 18 month) 
    and order_engagement_cnt >0  and order_cnt >0 )
    ...
    CREATE TABLE IF NOT EXISTS `bnile-cdw-prod.dag_migration_test.history_bn_pj_tagging`
    ( bnid	STRING
    ,Bucket STRING		
    ,run_date Date ) ;
    
    insert into `bnile-cdw-prod.dag_migration_test.history_bn_pj_tagging`
    
    select * from `bnile-cdw-prod.dag_migration_test.bn_pj_tagging`
    where run_date = current_date() and run_date not in 
    ( select max(run_date) from `bnile-cdw-prod.dag_migration_test.history_bn_pj_tagging` );
    
    
    --drop table if exists `bnile-cdw-prod.dag_migration_test.bn_pj_tag`;
    create or replace table `bnile-cdw-prod.dag_migration_test.bn_pj_tag` as
    ...
    from  `bnile-cdw-prod.dag_migration_test.bn_pj_tagging`
    )
    group by bnid
    ;
    ...
    --drop table if exists `bnile-cdw-prod.dag_migration_test.rfm_eng_src`;
    create or replace table `bnile-cdw-prod.dag_migration_test.rfm_eng_src`
    AS
    SELECT
    bnid, 
    ...
    FROM
    `bnile-cdw-prod.dag_migration_test.up_explicit_attributes`
    WHERE
    ...
    --DROP TABLE IF EXISTS `bnile-cdw-prod.dag_migration_test.rfm_eng`;
    CREATE or replace TABLE `bnile-cdw-prod.dag_migration_test.rfm_eng` AS
    SELECT
    bnid,
    ...
    FROM
    `bnile-cdw-prod.dag_migration_test.rfm_eng_src`
    ;
    
    -- Jewelry source data 
    --drop table if exists `bnile-cdw-prod.dag_migration_test.rfm_jewelry_src`;
    create or replace table `bnile-cdw-prod.dag_migration_test.rfm_jewelry_src`
    AS
    SELECT
    bnid, 
    ...
    --DROP TABLE IF EXISTS  `bnile-cdw-prod.dag_migration_test.rfm_jewelry`;
    CREATE or replace TABLE 
    `bnile-cdw-prod.dag_migration_test.rfm_jewelry` AS
    SELECT
    bnid,
    order_jewelry_cnt,
    ...
    FROM
    `bnile-cdw-prod.dag_migration_test.rfm_jewelry_src` 
    ;'''
    return bql