...
def jewellery_recommendation_product_scoring(params):
    """
    ...
    threshold = 0
    df5_total = df4.applymap(lambda x: 1 if x > threshold else 0)

    feature_matrix = sparse.csr_matrix(df5_total.values)

    # creating Model
    recommender = preprocess_product_features(df_bnid_offer_count, df_category).runMF(
    ...
    # # Ranking items on bases of price for each bnid
    final_df5["for_rank"] = (
...
if __name__ == "__main__":
    params = json.load(open("rc2_jewellery.json"))
    jewellery_recommendation_product_scoring(params)
