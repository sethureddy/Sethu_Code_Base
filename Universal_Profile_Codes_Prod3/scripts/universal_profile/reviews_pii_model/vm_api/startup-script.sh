...
mkdir /ds_model_run
cd /ds_model_run
sudo gsutil -m cp -r gs://$CS_BUCKET/dags/scripts/universal_profile/reviews_pii_model/* .
#sudo gsutil -m cp -r gs://$CS_BUCKET/$DIRECTORY_PATH/conf/* .
#sudo mv diamond/diamond_recommendation_user_product_scoring.py setting/settings_recommendation_user_product_scoring.py jewellery/jewellery_recommendation_user_product_scoring.py .
...
pip3 install fsspec==0.8.0


#updating reshape.py to int64 at num_cells
#sudo cp reshape.py /usr/local/lib/python3.6/dist-packages/pandas/core/reshape/


##user_recommendation scoring run
#sudo python3 user_recommendation_scoring.py

#recommendation output run
sudo python3 reviews_pii_final.py config.yml
