...
def diamond_recommendation_scoring(params):

    input_file = params["input_file"]
    output_path = params["output_path"]
    scoring_date = params["scoring_date"]

    # Reading output data from diamond_recommendation_data.py
    ...
    sql = """SELECT distinct lower(SKU) as SKU_ID, cast(USD_PRICE as int64) as avg_price FROM `{DIAMOND_PRICE}` where CAPTURE_DATE_KEY = {df_date} and usd_price is not null """.format(
        df_date=df.date_key.max(),
        DIAMOND_PRICE=DIAMOND_PRICE
    )
    df_price = client.query(sql).to_dataframe()
    ...
    # Shorlisting bnid for creating Recommendations
    ...
    # creation of interaction data within user and item based on view counts
    ...
    item_dict = preprocess(df_bnid_cat_count, df_category).create_item_dict(
        "DIAMOND_CATEGORY_IDENTIFIER", "DIAMOND_CATEGORY"
    )

    # creating Model
    recommender = preprocess(df_bnid_cat_count, df_category).runMF(interaction_matrix)

    # calling recommender model for recommending products
    ...
    # merging sku with scoring date prices
    ...
    # assigning rank to customers with no recommendations
...
if __name__ == "__main__":
    ...
    diamond_recommendation_scoring(params)
