...
class preprocess:
    ...
    def create_interaction_matrix(self, user_col, item_col, views):
        ...
        if preprocess.norm:
            interactions = interactions.applymap(
                lambda x: 1 if x > preprocess.threshold else 0
            )
        return interactions
    ...
    def runMF(self, interaction_matrix):
        ...
        model = LightFM(
            no_components=preprocess.n_components, loss=preprocess.loss, k=preprocess.k
        )
        model.fit(x, epochs=preprocess.epoch, num_threads=preprocess.n_jobs)
        return model

    def sample_recommendation_user(
        self, model, interactions, user_id, user_dict, item_dict, category_column
    ):
        """
        ...
        known_categories = list(
            pd.Series(
                interactions.loc[user_id, :][
                    interactions.loc[user_id, :] > preprocess.threshold_recom
                ].index
            ).sort_values(ascending=False)
        )
        scores = [x for x in scores if x not in known_categories]
        return_score_list = scores[0 : preprocess.nrec_items]
        return_score_df = scores_df[scores_df[category_column].isin(return_score_list)]
        known_categories = list(
            pd.Series(known_categories).apply(lambda x: item_dict[x])
        )
