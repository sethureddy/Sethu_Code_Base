def bn_sc_pj_attributes():
    ...
    drop table if exists `bnile-data-eng-dev.dag_migration_test.bn_sc_pj_attributes`;
    
    CREATE TABLE IF NOT EXISTS `bnile-data-eng-dev.dag_migration_test.bn_sc_pj_attributes` as
    ...
    WITH customer_data AS (
        ...
        ead.bnid
          ...
          ,prd.merch_product_category
            ...
            FROM `bnile-data-eng-dev.dssprod_o_warehouse.click_fact`  cf
            INNER JOIN `bnile-data-eng-dev.dssprod_o_warehouse.email_guid`  egl
                ON cf.GUID_KEY = egl.GUID_KEY
            LEFT JOIN `bnile-data-eng-dev.dssprod_o_warehouse.page_group_dim`  pgd
                ON cf.PAGE_GROUP_KEY = pgd.PAGE_GROUP_KEY
            LEFT JOIN `bnile-data-eng-dev.dssprod_o_warehouse.time_dim`  td
                ON cf.TIME_KEY = td.TIME_KEY
            LEFT JOIN `bnile-data-eng-dev.dssprod_o_warehouse.uri_dim`  ud
                ON cf.URI_KEY = ud.URI_KEY
           left join `bnile-data-eng-dev.o_product.offer_attributes_dim`  prd
           on cf.OFFER_KEY  = prd.offer_id
             join  `bnile-data-eng-dev.dag_migration_test.bn_guid_all` ead
...
where 
    ...
    and  bnid not in (select bnid from `bnile-data-eng-dev.dag_migration_test.stg_order_attributes` where order_cnt > 0)
 )  
    ),
    ...
    and	 bnid  in
       (
       select bnid from `bnile-data-eng-dev.dag_migration_test.stg_order_attributes`
     where
    ...
    --- Rule 1 - Logic for last prior purchases
        and 
          bnid  in (select bnid from `bnile-data-eng-dev.dag_migration_test.stg_order_attributes` where order_cnt > 0)
          group by bnid
     ) ) ,
    ...
    from `bnile-data-eng-dev.dssprod_o_warehouse.click_fact`   cf
    INNER JOIN
        `bnile-data-eng-dev.dssprod_o_warehouse.email_guid`  egl
      ON
        cf.GUID_KEY = egl.GUID_KEY
        left join
        `bnile-data-eng-dev.dag_migration_test.bn_guid_all`  ead
    ...
    from `bnile-data-eng-dev.up_tgt_ora_tables.freshdesk_ticket` fr
    left join `bnile-data-eng-dev.dag_migration_test.up_email_address_dim_d`  ead
    ...
    INNER JOIN
      ...
      on co.bnid = ead.bnid
        left join
        `bnile-data-eng-dev.dssprod_o_warehouse.page_group_dim`   pgd
        on cf.PAGE_GROUP_KEY = pgd.PAGE_GROUP_KEY
         JOIN
        `bnile-data-eng-dev.dssprod_o_warehouse.daily_product_fact` dpf
      ON
    ...
    from (select * from `bnile-data-eng-dev.dssprod_o_warehouse.click_fact` )  cf
    INNER JOIN
        `bnile-data-eng-dev.dssprod_o_warehouse.email_guid`  egl
      ON
        cf.GUID_KEY = egl.GUID_KEY
      ...
      on co.bnid = ead.bnid
        ...
        on cf.PAGE_GROUP_KEY = pgd.PAGE_GROUP_KEY
         JOIN
        `bnile-data-eng-dev.dssprod_o_diamond.diamond_daily_data` ddd
      ON
    ...
    (
    select bnid,coalesce(gender,derived_gender) as e_sc_gender from `bnile-data-eng-dev.dag_migration_test.bn_customer_profile`  where derived_gender <> 'N/A' )
    ...
    (SELECT distinct bnid, max(proposal_date) as e_sc_proposal_date from `bnile-data-eng-dev.dag_migration_test.up_explicit_attributes` 
    group by bnid
    ) proposal
    on 
    ...
    (select bnid ,max(wedding_date) as e_sc_wedding_date from `bnile-data-eng-dev.dag_migration_test.stg_pii_attributes` where wedding_date is not null 
    group by bnid
    ) wed_date
    ...
    (select bnid,e_sc_partner_email_address from
     ...
     as row_num
      from `bnile-data-eng-dev.dssprod_o_warehouse.email_address` e
      left join `bnile-data-eng-dev.dag_migration_test.up_email_address_dim_d`  ead
    ...
    FROM `bnile-data-eng-dev.dag_migration_test.stg_order_attributes` 
    group by bnid
     ) en
    ...
    distinct
               ...
               trim(lower(ead.email_address)) email_address ,
                ...
                from
                `bnile-data-eng-dev.dssprod_o_warehouse.product_order_fact` pof,
                `bnile-data-eng-dev.dssprod_o_warehouse.order_type_dim` otd,
                `bnile-data-eng-dev.dssprod_o_warehouse.product_dim` prd,
                `bnile-data-eng-dev.dssprod_o_warehouse.host_dim` hsd,
                `bnile-data-eng-dev.dssprod_o_warehouse.email_address_dim` ead,
                `bnile-data-eng-dev.dag_migration_test.up_email_address_dim_d` ed
                where 1=1
    ...
    FROM `bnile-data-eng-dev.dag_migration_test.up_explicit_attributes` 
    group by bnid
      ) email_send
      ...
      (SELECT bnid,max_date, max(case when row_num =1 then propensity_phase else null end ) as i_sc_pj_phase
        ...
        (select distinct bnid, propensity_phase,max_date,
          ...
          (SELECT distinct bnid,propensity_phase , max(model_run_date ) over (partition by bnid,propensity_phase) max_date,
            FROM `bnile-data-eng-dev.dag_migration_test.bn_sc_propensity`
            )
            )
    ...
    FROM
      `bnile-data-eng-dev.dag_migration_test.bn_sc_propensity` )
