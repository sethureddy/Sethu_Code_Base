...
def settings_recommendation_user_product_scoring(params):
    """
    recommmendation of settings on the basis
    ...
    threshold = 0
    df_item_sparse = df_group_offer_item.applymap(lambda x: 1 if x > threshold else 0)
    feature_matrix = scipy.sparse.csr_matrix(df_item_sparse.values)

    # creating User-Features (User Matrix)
    df_user_feat = df_main[["bnid", "gender", "add_to_basket", "item_purchase"]]

    ls_user_feat = ["gender", "add_to_basket", "item_purchase"]
    ...
    threshold = 0
    df_user_sparse = df_group_bnid_user.applymap(lambda x: 1 if x > threshold else 0)
    user_matrix = scipy.sparse.csr_matrix(df_user_sparse.values)

    # creating Model
    recommender = preprocess_user_features(df_bnid_cat_count, df_category).runMF(
    ...
    # Appending final recommendations
    ...
    final_result.to_csv(
        output_path
        + "settings_recommendation_user_product_feature"
        + scoring_date
        + ".csv"
    )
...
if __name__ == "__main__":
    ...
    settings_recommendation_user_product_scoring(params)