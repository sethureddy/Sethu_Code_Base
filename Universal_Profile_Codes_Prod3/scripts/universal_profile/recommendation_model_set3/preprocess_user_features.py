...
class preprocess_user_features:
    ...
    def runMF(self,interaction_matrix,feature_matrix,user_matrix):
        ...
        Required Input -
            ...
            - feature_matrix = contains the product information
            - user_matrix = contains the user information
        Expected Output  -
            Model - Trained model
        '''
    ...
    def sample_recommendation_user(self,model, interactions,feature_matrix,user_matrix,user_id,user_dict,item_dict,category_column):
        '''
        ...
        scores = pd.Series(model.predict(user_x,np.arange(n_items),item_features=feature_matrix,user_features=user_matrix))
        ...
        known_categories = list(pd.Series(interactions.loc[user_id,:][interactions.loc[user_id,:] > preprocess_user_features.threshold_recom].index).sort_values(ascending=False))
        scores = [x for x in scores if x not in known_categories]
        return_score_list = scores[0:preprocess_user_features.nrec_items]
