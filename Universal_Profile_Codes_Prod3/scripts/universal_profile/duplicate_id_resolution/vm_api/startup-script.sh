...
mkdir /duplicate_id_resolution
cd /duplicate_id_resolution
sudo gsutil -m cp -r gs://$CS_BUCKET/dags/scripts/universal_profile/duplicate_id_resolution/* .
sudo gsutil -m cp -r gs://$CS_BUCKET/$DIRECTORY_PATH/conf/* .
echo "y" | sudo apt install python3-pip
sudo pip3 install -r requirements.txt
sudo pip3 install sparse-dot-topn
