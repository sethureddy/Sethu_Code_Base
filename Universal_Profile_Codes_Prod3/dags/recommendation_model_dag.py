...
from scripts.universal_profile.recommendation_model.model_task_preprocess import (
...
GCS_DIR_PATH = "data/recommendation_model_deploy"
VM_CREATION_CODE_PATH = "/home/airflow/gcs/dags/scripts/universal_profile/recommendation_model/vm_api"
VM_INSTANCE_ZONE = "us-central1-a"
VM_INSTANCE_NAME = "ds-model-deploy-automation-" + EXECUTION_DATE
...
FINAL_INTERMEDIATE_TABLE = (
    "o_customer.bn_product_recommendation_intermediate" + EXECUTION_DATE
)
...
with models.DAG(
    ...
    task_diamond_export_query_data_gcs = bigquery_to_gcs.BigQueryToCloudStorageOperator(
        task_id="diamond_export_query_data_gcs",
    ...
    task_append_final_csv_output_to_table = gcs_to_bq.GoogleCloudStorageToBigQueryOperator(
        task_id="append_final_csv_output_to_table",
    ...
    task_insert_from_intermediate_to_final = bigquery_operator.BigQueryOperator(
        ...
        sql="""delete from `{PROJECT_ID}.{FINAL_TABLE_OUTPUT}` where MODEL_SEQ=1 and run_date=(
...
insert into `{PROJECT_ID}.{FINAL_TABLE_OUTPUT}`
    ...
    subject='UP ETL Alert:Success(bn_recommendation_set_1)',
    html_content=''' 
    Hi,<br>
