...
from airflow.contrib.operators import bigquery_operator
from scripts.universal_profile.bnid_generation_preprocess import bnid_generation_preprocess
from scripts.universal_profile.bnid_generation_postprocess import bnid_generation_postprocess
...
with DAG('bn_up_bnid_generation', schedule_interval=None, default_args=default_args) as dag:
    ...
    start_id_resolution = TriggerDagRunOperator(
        dag=dag,
        task_id='start_id_resolution',
        trigger_dag_id="id_resolution_dag",
        params={'message': 'Hello World'}
    )
    
    task_check_id_resolution_data = bash_operator.BashOperator(
        task_id='check_id_resolution_data',
        bash_command="python3 /home/airflow/gcs/dags/scripts/universal_profile/id_resolution_data_check.py",
        dag=dag
    )
    ...
    task_merge_id_resolution_data_to_bnid_dedup_map = bigquery_operator.BigQueryOperator(
        ...
        sql= """create or replace table bnile-cdw-prod.up_stg_dedup.bnid_dedup_map_probablistic as 
                select * from bnile-cdw-prod.up_stg_dedup.bnid_dedup_map;
                
                insert into bnile-cdw-prod.up_stg_dedup.bnid_dedup_map_probablistic
                select primary_bnid		
                ...
                create or replace table bnile-cdw-prod.up_stg_dedup.bnid_dedup_map_probablistic as
                (select * from bnile-cdw-prod.up_stg_dedup.bnid_dedup_map_probablistic
    ...
    task_bnid_generation_id_merge = bash_operator.BashOperator(
        ...
        bash_command="python3 /home/airflow/gcs/dags/scripts/universal_profile/bnid_generation_id_merge.py",
        dag=dag
    )
...
task_bnid_generation_preprocess >> start_id_resolution >> task_check_id_resolution_data >> task_merge_id_resolution_data_to_bnid_dedup_map >> task_bnid_generation_id_merge >> task_bnid_generation_postprocess
...
start_bn_master_up_process = TriggerDagRunOperator(
    ...
    trigger_dag_id="bn_master_up_process",
